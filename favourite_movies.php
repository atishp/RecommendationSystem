...
function unLikeMovieFunction(item_id){
...
<img height="20" width="20" src="images/recsys/ssm.gif" />
    ...
    <div class="panel panel-danger">
            <div class="panel-heading">
                           <center> <h3 >Favourite Movies</h3> </center>
...
include("config.php"); 
 ...
 $query=" SELECT * FROM `universal_movies` WHERE `id` IN
 (SELECT `item_id` FROM $username WHERE `item_domain`='movie')";
 
	$result=mysql_query($query);
		
	if(($row=mysql_fetch_assoc($result))== NULL )
{
echo "<h3> <font color='gray' > You have not Liked any Movies Yet . . . ! ! ! </font> </h3>";
echo "<h3> <font color='gray' > Try this: </font> </h3>
	<h4> <font color='blue' > <a href= 'trending.php'> top trending</a> </font> </h4>
	<h4> <font color='blue' > <a href= 'recommend_movies.php'> My movies</a> </font> </h4>";
...
else
{
	...
	<tr align='center'>
		<td><img src='".$row["image"]."' width='100px' height='100px'></td>
				<td><h3>".$row["name"]."</h3><br><input type=\"submit\" id= ".$row["id"]." name=\"unlike\" value=\"unlike\" onClick='unLikeMovieFunction(".$row["id"].")'>
